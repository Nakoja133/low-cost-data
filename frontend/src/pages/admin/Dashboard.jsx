...
const AdminDashboard = () => {
  ...
  // ✅ FIXED: StatCard with organized layout (Label Top, Value Bottom-Left, Sub Bottom-Right)
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)' }}>
      ...
      <main className="main-content">
        ...
        <div className="stats-grid">
          ...
          <StatCard label="Platform Profit" value={`GH₵ ${stats.adminProfit.toFixed(2)}`} color="#10b981" 
            sub={`Margin ${stats.platformMargin}% · Paid out GH₵ ${stats.totalPayouts.toFixed(2)}`} 
          />
      ...
      <style>{`
        ...
        .quick-btn-icon { font-size: 1.35rem; margin-bottom: 0.4rem; }
        .quick-btn-label { font-weight: 700; font-size: 0.85rem; margin-bottom: 0.15rem; }
        .quick-btn-sub { font-size: 0.7rem; color: var(--text-secondary); }
        ...
        .status-item { padding: 0.875rem 1rem; border-radius: 0.75rem; }
        .status-label { font-weight: 700; font-size: 0.85rem; margin-bottom: 0.2rem; }
        .status-sub { font-size: 0.75rem; color: var(--text-secondary); }

        /* ── Modal ──────────────────────────────────────── */
        .modal-overlay {
          position: fixed; inset: 0; background: rgba(0,0,0,0.72); z-index: 998;
          display: flex; align-items: center; justify-content: center; padding: 1rem;
          backdrop-filter: blur(4px);
        }
        .modal-card {
          background: var(--card-bg); border-radius: 1.25rem; max-width: 520px; width: 100%;
          border: 1px solid var(--border-color); box-shadow: 0 25px 60px rgba(0,0,0,0.4);
          overflow: hidden; max-height: 90dvh; display: flex; flex-direction: column;
          animation: bounceIn 0.32s cubic-bezier(0.34,1.56,0.64,1) both;
        }
        .modal-header { padding: 1.25rem 1.5rem; color: white; display: flex; justify-content: space-between; align-items: flex-start; }
        .holding-header { background: linear-gradient(135deg, #6366f1, #8b5cf6); }
        .modal-icon { font-size: 1.5rem; margin-bottom: 0.2rem; }
        .modal-title { font-weight: 800; font-size: 1.05rem; margin: 0; }
        .modal-subtitle { opacity: 0.88; font-size: 0.8rem; margin: 0.2rem 0 0; }
        .modal-close { background: rgba(255,255,255,0.2); border: none; border-radius: 50%; width: 34px; height: 34px; color: white; cursor: pointer; display: flex; align-items: center; justify-content: center; }
        .modal-body { padding: 1.25rem 1.5rem; overflow-y: auto; }

        .holding-summary { display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 0.75rem; margin-bottom: 1.25rem; }
        .holding-card { background: var(--bg-secondary); padding: 0.875rem; border-radius: 0.75rem; border: 1px solid var(--border-color); }
        .holding-card-label { font-size: 0.7rem; color: var(--text-secondary); margin-bottom: 0.3rem; }
        .holding-card-value { font-size: 1.05rem; font-weight: 800; }
        
        .agent-list { display: flex; flex-direction: column; gap: 0.5rem; }
        .agent-row { display: flex; justify-content: space-between; align-items: center; padding: 0.75rem 0.875rem; background: var(--bg-secondary); border-radius: 0.625rem; border: 1px solid var(--border-color); }
        .agent-name { font-weight: 600; font-size: 0.875rem; }
        .agent-email { font-size: 0.72rem; color: var(--text-secondary); }
        .agent-balance { font-weight: 800; color: var(--success, #10b981); }

        /* ── Animations ─────────────────────────────────── */
        @keyframes bounceIn { from { opacity: 0; transform: scale(0.87); } to { opacity: 1; transform: scale(1); } }
        @keyframes fadeSlideDown { from { opacity: 0; transform: translateY(-10px); } to { opacity: 1; transform: translateY(0); } }

        .main-content { padding: 1.5rem; }
        @media (max-width: 767px) { .main-content { padding: 1rem 0.75rem; } }
      `}</style>
    </div>
  );
};
