...
const Users = () => {
      ...
      <div style={{ display:'flex', alignItems:'flex-end', gap:'3px', height:'80px', paddingBottom:'16px' }}>
        {data.map((item, i) => (
          <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', height:'100%', justifyContent:'flex-end' }}>
            <div title={`${item.label}: ${item.value}`} style={{ width:'100%', background:'linear-gradient(180deg,var(--primary),#4f46e5)', borderRadius:'3px 3px 0 0', height:`${Math.max((item.value/max)*100,3)}%`, minHeight:'3px', transition:'height 0.4s' }} />
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* ── Agent Stats Modal ──────────────────────────────── */}
      {selectedAgent && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.72)', zIndex:998, display:'flex', alignItems:'flex-start', justifyContent:'center', padding:'1rem', paddingTop:'4rem', overflowY:'auto' }}>
          <div style={{ background:'var(--card-bg)', borderRadius:'1.25rem', maxWidth:'560px', width:'100%', border:'1px solid var(--border-color)', boxShadow:'0 25px 60px rgba(0,0,0,0.4)', overflow:'hidden', animation:'popIn 0.3s cubic-bezier(0.34,1.56,0.64,1)' }}>
            ...
            <div style={{ background:'linear-gradient(135deg,#6366f1,#8b5cf6)', padding:'1.25rem 1.5rem', color:'white', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
              <div>
                ...
                <h3 style={{ fontWeight:'800', fontSize:'1.05rem', margin:0 }}>{selectedAgent.username || selectedAgent.email}</h3>
      ...
      <main className="main-content" style={{ paddingTop:'1rem' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.5rem', flexWrap:'wrap', gap:'1rem' }}>
          <div>
            <h2 style={{ fontSize:'1.4rem', fontWeight:'800' }}>👥 User Management</h2>
            <p style={{ color:'var(--text-secondary)', fontSize:'0.875rem' }}>Click an agent row to view their statistics</p>
          </div>
        ...
        {/* Create form */}
        {showForm && (
          <div className="card" style={{ marginBottom:'1.5rem', animation:'fadeSlide 0.25s ease' }}>
            <h3 style={{ fontWeight:'700', marginBottom:'1.25rem' }}>✨ Create {formData.role==='admin'?'Admin':'Agent'} Account</h3>
            <form ref={formRef} onSubmit={handleSubmit} autoComplete="off"
              ...
              <div><label style={labelSt}>Username</label>
              ...
              <div><label style={labelSt}>Email *</label>
              ...
              <div><label style={labelSt}>Password *</label>
              ...
              <div><label style={labelSt}>Phone Number</label>
              ...
              {formData.role === 'agent' && (
                <div><label style={labelSt}>Store Name</label>
            ...
            <table>
              ...
              <tbody>
                {users.filter(u => {
                  ...
                  <tr><td colSpan="8" style={{ textAlign:'center', padding:'2.5rem', color:'var(--text-muted)' }}>No users match your search</td></tr>
                ) : users.filter(u => {
                  if (!search.trim()) return true;
                  const query = search.trim().toLowerCase();
                  return [u.username, u.store_name, u.phone].some(val => val?.toString().toLowerCase().includes(query));
                  ...
                  <tr key={u.id}
                    ...
                    style={{ cursor:u.role==='agent'?'pointer':'default', transition:'background 0.15s' }}
                  ...
                  >
                    <td>
                      ...
                      {u.role === 'agent' && <div style={{ fontSize:'0.68rem', color:'var(--primary)', marginTop:'0.1rem' }}>Click to view stats →</div>}
                    </td>
                    <td style={{ fontSize:'0.875rem' }}>{u.phone || <span style={{ color:'var(--text-muted)' }}>—</span>}</td>
                    <td>
                    ...
                    <td>
                      {u.store_slug ? (
                        <div onClick={e => e.stopPropagation()}>
                          <div style={{ fontSize:'0.85rem', fontWeight:'500' }}>{u.store_name || u.store_slug}</div>
                    ...
                    <td style={{ fontWeight:'600' }}>GH₵ {parseFloat(u.total_sales).toFixed(2)}</td>
                    <td style={{ color:'var(--text-secondary)', fontSize:'0.82rem' }}>{new Date(u.created_at).toLocaleDateString()}</td>
