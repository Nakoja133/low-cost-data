...
const Store = () => {
  ...
  const fetchStore = async () => {
    try { const r = await api.get(`/store/${slug}`); setStore(r.data); }
  ...
  const handleTrackOrder = async (e) => {
    ...
    try {
      const r = await api.get(`/store/${slug}/orders/track`, { params:{ phone:trackPhone, name:trackName||undefined } });
      setTrackedOrder(r.data);
    } catch { setTrackError('No orders found for this phone number'); }
  };

  const getGB = (desc) => { const m = desc?.match(/(\d+\.?\d*)\s*GB/i); return m ? m[1]+' GB' : desc; };
  ...
  if (!store) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem', background:'var(--bg-primary)' }}>
      <div style={{ textAlign:'center' }}>
        ...
        <p style={{ color:'var(--text-secondary)' }}>This store doesn't exist or has been deactivated.</p>
      </div>
    </div>
  );

  const grouped = store.packages.reduce((acc, p) => { if (!acc[p.network]) acc[p.network]=[]; acc[p.network].push(p); return acc; }, {});
  ...
  const whatsappUrl  = store.agent.whatsapp_number ? `https://wa.me/${store.agent.whatsapp_number.replace(/[^0-9]/g,'')}?text=Hello%20${encodeURIComponent(storeName)}!%20I%20want%20to%20buy%20a%20data%20bundle.` : null;
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg-primary)', paddingBottom:'3rem' }}>
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', zIndex:999, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
          <div style={{ background:'var(--card-bg)', borderRadius:'1.25rem', maxWidth:'380px', width:'100%', overflow:'hidden', boxShadow:'0 25px 60px rgba(0,0,0,0.4)', animation:'popIn 0.25s cubic-bezier(0.34,1.56,0.64,1)' }}>
            ...
            <div style={{ padding:'1.5rem' }}>
              {[['Package',getGB(pkg.description)],['Network',pkg.network],['Phone',customerPhone],['Amount',`GH₵ ${bundlePrice.toFixed(2)}`]].map(([k,v]) => (
              ...
              <div style={{ display:'flex', gap:'0.75rem', marginTop:'1.25rem' }}>
                <button onClick={() => setShowConfirm(false)} style={{ flex:1, padding:'0.875rem', background:'var(--bg-tertiary)', border:'1px solid var(--border-color)', borderRadius:'0.625rem', cursor:'pointer', fontWeight:'600', color:'var(--text-primary)' }}>Cancel</button>
            ...
            <div style={{ padding:'1.25rem', display:'flex', flexDirection:'column', gap:'0.75rem' }}>
              {whatsappUrl && (
                <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" onClick={() => setShowContact(false)}
                  ...
                  <div><div>Chat on WhatsApp</div><div style={{ fontSize:'0.75rem', fontWeight:'400', opacity:0.8 }}>Opens WhatsApp chat</div></div>
                </a>
              )}
              {callUrl && (
                <a href={callUrl} onClick={() => setShowContact(false)}
                  ...
                  <span style={{ fontSize:'1.5rem' }}>📞</span>
                  <div><div>Call {store.agent.phone}</div><div style={{ fontSize:'0.75rem', fontWeight:'400', opacity:0.8 }}>Dials seller's number</div></div>
                </a>
              )}
              {!whatsappUrl && !callUrl && <p style={{ textAlign:'center', color:'var(--text-muted)', padding:'1rem' }}>No contact info available</p>}
              <button onClick={() => setShowContact(false)} style={{ padding:'0.875rem', background:'var(--bg-tertiary)', border:'1px solid var(--border-color)', borderRadius:'0.625rem', color:'var(--text-primary)', cursor:'pointer', fontWeight:'600' }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', zIndex:999, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
          <div style={{ background:'var(--card-bg)', borderRadius:'1.25rem', maxWidth:'360px', width:'100%', overflow:'hidden', boxShadow:'0 25px 60px rgba(0,0,0,0.4)', animation:'popIn 0.25s cubic-bezier(0.34,1.56,0.64,1)' }}>
            ...
            <div style={{ padding:'1.5rem', display:'flex', flexDirection:'column', gap:'0.5rem' }}>
              {[['Store Name',storeName],['Email',store.agent.email],...(store.agent.phone?[['Phone',store.agent.phone]]:[])].map(([k,v]) => (
                <div key={k} style={{ display:'flex', justifyContent:'space-between', padding:'0.625rem 0', borderBottom:'1px solid var(--border-color)' }}>
              ...
              <button onClick={() => setShowAbout(false)} style={{ marginTop:'0.5rem', padding:'0.875rem', background:'var(--bg-tertiary)', border:'1px solid var(--border-color)', borderRadius:'0.625rem', color:'var(--text-primary)', cursor:'pointer', fontWeight:'600' }}>Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ background:'linear-gradient(135deg,#4f46e5 0%,#7c3aed 100%)', padding:'1.5rem 1.25rem 1.75rem', color:'white', position:'relative', overflow:'hidden' }}>
        ...
        {/* ✅ Greeting + Contact Seller side by side */}
            ...
            <div style={{ background:'var(--card-bg)', borderRadius:'1rem', padding:'1.5rem', border:'1px solid var(--border-color)' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:'1.125rem' }}>
                <div>
                  ...
                  <input type="text" value={customerName} onChange={e => setCustomerName(e.target.value)} placeholder="Kwame Mensah" className="input" />
                </div>
                <div>
                  ...
                  <input type="tel" value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} placeholder="0549722133" className="input" required />
                </div>
                {pkg && (
          ...
          <div style={{ background:'var(--card-bg)', borderRadius:'1rem', padding:'1.5rem', border:'1px solid var(--border-color)' }}>
            ...
            <form onSubmit={handleTrackOrder} style={{ display:'flex', flexDirection:'column', gap:'1.125rem' }}>
              <div>
                ...
                <input type="tel" value={trackPhone} onChange={e => setTrackPhone(e.target.value)} placeholder="0549722133" className="input" required />
              </div>
              <div>
                ...
                <input type="text" value={trackName} onChange={e => setTrackName(e.target.value)} placeholder="Kwame Mensah" className="input" />
              </div>
            ...
            {trackedOrder && (
              <div style={{ marginTop:'1.5rem', background:'var(--bg-secondary)', borderRadius:'0.875rem', overflow:'hidden', border:'1px solid var(--border-color)' }}>
                ...
                <div style={{ padding:'1.125rem 1.25rem', display:'flex', flexDirection:'column', gap:'0.75rem' }}>
                  {[['Package',getGB(trackedOrder.package_name)],['Phone',trackedOrder.customer_phone],...(trackedOrder.customer_name?[['Name',trackedOrder.customer_name]]:[]),['Paid',`GH₵ ${parseFloat(trackedOrder.amount_paid).toFixed(2)}`]].map(([k,v]) => (
      ...
      <div style={{ textAlign:'center', padding:'2rem 1rem 1rem', color:'var(--text-muted)', fontSize:'0.78rem', borderTop:'1px solid var(--border-color)', marginTop:'2rem' }}>
        <p>Powered by Low-Cost Data Bundles</p><p style={{ fontSize:'0.7rem', marginTop:'0.2rem' }}>© 2026 All rights reserved</p>
      </div>
