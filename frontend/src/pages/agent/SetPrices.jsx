...
const SetPrices = () => {
  ...
  const [bulkScope,  setBulkScope]  = useState('all'); // 'all' | 'MTN' | 'Telecel' | 'AirtelTigo'
  ...
  const handleMarkupChange = (pkgId, val) => {
    ...
    const pct  = isNaN(raw) ? '' : Math.max(0, raw).toString(); // no negatives
    const base = prices[pkgId]?.base_price || 0;
    ...
    <div className="dashboard-container">
      ...
      <main className="main-content" style={{ paddingTop: '1rem' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', borderRadius: '1rem', padding: '1.5rem', marginBottom: '1.5rem', color: 'white' }}>
          <h2 style={{ fontSize: '1.4rem', fontWeight: '800', marginBottom: '0.25rem' }}>💰 Set Bundle Prices</h2>
          <p style={{ opacity: 0.9, fontSize: '0.875rem' }}>Set your selling price above the base price to earn profit on each sale</p>
        </div>

        {/* Mode Toggle */}
        <div className="card" style={{ marginBottom: '1.25rem' }}>
          <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginBottom: '0.75rem', fontWeight: '600' }}>Pricing Method</p>
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem' }}>
            {[['manual', '✏️ Manual Price'], ['markup', '📊 Markup %']].map(([val, lbl]) => (
              <button key={val} onClick={() => setMode(val)} style={{ padding: '0.625rem 1.25rem', border: 'none', borderRadius: '0.5rem', cursor: 'pointer', fontWeight: '600', fontSize: '0.875rem', background: mode === val ? 'var(--primary)' : 'var(--bg-tertiary)', color: mode === val ? 'white' : 'var(--text-primary)', transition: 'all 0.2s' }}>
                {lbl}
              </button>
            ))}
          </div>
          ...
          {mode === 'markup' && (
            <div style={{ marginTop: '1rem', padding: '1rem', background: 'var(--bg-secondary)', borderRadius: '0.75rem', border: '1px solid var(--border-color)', animation: 'fadeSlide 0.2s ease' }}>
              <p style={{ fontWeight: '700', fontSize: '0.875rem', marginBottom: '0.875rem' }}>🚀 Apply Bulk Markup</p>
              <div style={{ display: 'flex', gap: '0.625rem', flexWrap: 'wrap', marginBottom: '0.875rem' }}>
                {['all', ...NETWORKS].map(scope => (
                  <button key={scope} onClick={() => setBulkScope(scope)} style={{ padding: '0.45rem 0.875rem', border: 'none', borderRadius: '9999px', cursor: 'pointer', fontWeight: '600', fontSize: '0.8rem', background: bulkScope === scope ? (scope === 'all' ? 'var(--primary)' : NET_COLORS[scope]) : 'var(--bg-tertiary)', color: bulkScope === scope ? 'white' : 'var(--text-primary)', transition: 'all 0.2s' }}>
                    {scope === 'all' ? '🌐 All Networks' : `${NET_ICONS[scope]} ${scope}`}
                  </button>
                ))}
              </div>
              <div style={{ display: 'flex', gap: '0.625rem', alignItems: 'center' }}>
                <input type="number" step="0.5" min="0" value={bulkMarkup} onChange={e => setBulkMarkup(e.target.value)} placeholder="e.g. 10" style={{ flex: 1, padding: '0.625rem 0.875rem', border: '1px solid var(--border-color)', borderRadius: '0.5rem', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: '0.95rem' }} />
                <span style={{ color: 'var(--text-secondary)', fontWeight: '600' }}>%</span>
                <button onClick={applyBulkMarkup} style={{ padding: '0.625rem 1.25rem', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: 'white', border: 'none', borderRadius: '0.5rem', cursor: 'pointer', fontWeight: '700', fontSize: '0.875rem', whiteSpace: 'nowrap' }}>
        ...
        {/* Message */}
        {message.text && (
          <div style={{ padding: '0.875rem 1rem', borderRadius: '0.5rem', marginBottom: '1.25rem', background: message.type === 'success' ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', border: `1px solid ${message.type === 'success' ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)'}`, color: message.type === 'success' ? 'var(--success)' : 'var(--danger)', fontWeight: '500' }}>
            {message.text}
          </div>
        )}
        ...
        {NETWORKS.map(net => {
          ...
          return (
            <div key={net} className="card" style={{ marginBottom: '1.25rem', borderLeft: `4px solid ${NET_COLORS[net]}` }}>
              <h3 style={{ fontSize: '1rem', fontWeight: '700', color: NET_COLORS[net], marginBottom: '1rem' }}>
                {NET_ICONS[net]} {net}
              </h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {pkgs.map(pkg => {
                  ...
                  return (
                    <div key={pkg.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: '0.75rem', alignItems: 'center', padding: '0.875rem', background: 'var(--bg-secondary)', borderRadius: '0.75rem', border: `1px solid ${isValid ? 'var(--border-color)' : 'rgba(239,68,68,0.4)'}` }}>
                      {/* Package info */}
                      <div>
                      ...
                      {/* Profit badge */}
                      <div style={{ padding: '0.35rem 0.625rem', borderRadius: '0.375rem', fontSize: '0.75rem', fontWeight: '700', background: parseFloat(profit) > 0 ? 'rgba(16,185,129,0.15)' : 'var(--bg-tertiary)', color: parseFloat(profit) > 0 ? 'var(--success)' : 'var(--text-muted)', whiteSpace: 'nowrap' }}>
        ...
        {/* Save */}
        <button onClick={handleSave} disabled={saving} style={{ width: '100%', padding: '1rem', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: 'white', border: 'none', borderRadius: '0.75rem', fontWeight: '700', fontSize: '1rem', cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1, transition: 'all 0.2s', boxShadow: '0 4px 15px rgba(99,102,241,0.3)' }}>
