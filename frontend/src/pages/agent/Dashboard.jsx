...
const AgentDashboard = () => {
  ...
  useEffect(() => {
    if (user?.terms_accepted === false) {
      api.get('/agent/terms').then(r => setTermsContent(r.data.terms || '')).catch(() => setTermsContent('Unable to load.'));
      setShowTerms(true);
    } else {
      loadAll();
    }
  ...
  const loadAll = async () => {
    try {
      ...
      if (s.status === 'fulfilled') {
        ...
        setStats({ totalOrders:d.totalOrders||0, totalRevenue:d.totalRevenue||0, totalProfit:d.totalProfit||0 });
      }
  ...
  const dismissNotif = async () => {
    ...
    try { await api.post('/agent/notifications/dismiss', { type:n.type }); } catch {}
    if (notifIdx < notifications.length - 1) setNotifIdx(i => i + 1);
    else { setShowNotif(false); setNotifications([]); }
  };

  const copyLink = () => { navigator.clipboard.writeText(storeUrl); setLinkCopied(true); setTimeout(() => setLinkCopied(false), 2500); };
  const greeting = () => {
    const h = new Date().getHours(); const n = user?.username || user?.email?.split('@')[0] || 'Agent';
  ...
  const getNotifIcon  = (n) => ({ terms:'📋', min_withdrawal:'💰', package_update:'📦', link_update:'🔗' }[n.type] || '🔔');
  const getNotifColor = (n) => ({ terms:'linear-gradient(135deg,#6366f1,#8b5cf6)', min_withdrawal:'linear-gradient(135deg,#f59e0b,#d97706)', package_update:'linear-gradient(135deg,#10b981,#059669)', link_update:'linear-gradient(135deg,#3b82f6,#2563eb)' }[n.type] || 'linear-gradient(135deg,#6366f1,#8b5cf6)');
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg-primary)', display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
      <div style={{ background:'var(--card-bg)', borderRadius:'1.25rem', width:'100%', maxWidth:'600px', border:'1px solid var(--border-color)', boxShadow:'0 25px 60px rgba(0,0,0,0.4)', overflow:'hidden' }}>
        <div style={{ background:'linear-gradient(135deg,#6366f1,#8b5cf6)', padding:'1.75rem 2rem', color:'white' }}>
          ...
          <h2 style={{ fontWeight:'800', fontSize:'1.4rem' }}>Agent Terms & Conditions</h2>
          <p style={{ opacity:0.9, fontSize:'0.875rem' }}>Please read before accessing your dashboard</p>
        </div>
        <div style={{ padding:'1.5rem 2rem', maxHeight:'45vh', overflowY:'auto', borderBottom:'1px solid var(--border-color)' }}>
          <pre style={{ whiteSpace:'pre-wrap', fontFamily:'inherit', fontSize:'0.9rem', lineHeight:'1.9', color:'var(--text-primary)', margin:0 }}>{termsContent}</pre>
        </div>
        <div style={{ padding:'1.5rem 2rem' }}>
          <p style={{ fontSize:'0.78rem', color:'var(--text-muted)', textAlign:'center', marginBottom:'1rem' }}>Accepting means you agree to follow all rules. Declining logs you out.</p>
          <div style={{ display:'flex', gap:'0.75rem' }}>
            <button onClick={() => { logout(); navigate('/login'); }} style={{ flex:1, padding:'0.875rem', background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.3)', borderRadius:'0.625rem', color:'var(--danger)', fontWeight:'700', cursor:'pointer' }}>✕ Decline</button>
            <button onClick={acceptTerms} disabled={accepting} style={{ flex:2, padding:'0.875rem', background:'linear-gradient(135deg,#6366f1,#8b5cf6)', border:'none', borderRadius:'0.625rem', color:'white', fontWeight:'800', cursor:'pointer', opacity:accepting?0.7:1 }}>{accepting?'Saving...':'✓ Accept & Enter'}</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="dashboard-container">
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.65)', zIndex:998, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
          <div style={{ background:'var(--card-bg)', borderRadius:'1.25rem', maxWidth:'400px', width:'100%', border:'1px solid var(--border-color)', boxShadow:'0 20px 60px rgba(0,0,0,0.4)', overflow:'hidden', animation:'bounceIn 0.4s cubic-bezier(0.34,1.56,0.64,1)' }}>
            <div style={{ background:getNotifColor(curNotif), padding:'1.25rem 1.5rem', color:'white' }}>
              ...
              <p style={{ fontWeight:'800', fontSize:'1rem', margin:0 }}>{greeting().split(',')[0]}, {user?.username || user?.email?.split('@')[0]}!</p>
            </div>
            <div style={{ padding:'1.25rem 1.5rem' }}>
              ...
              {(curNotif.type === 'terms') && (
                <button onClick={() => { dismissNotif(); navigate('/agent/terms'); }} style={{ display:'flex', alignItems:'center', gap:'0.5rem', color:'var(--primary)', background:'rgba(99,102,241,0.1)', border:'1px solid rgba(99,102,241,0.3)', borderRadius:'0.5rem', padding:'0.625rem 1rem', cursor:'pointer', fontWeight:'700', fontSize:'0.875rem', marginBottom:'0.75rem', width:'100%', justifyContent:'center' }}>📋 Read Rules →</button>
              )}
              {(curNotif.type === 'link_update') && (groupLink || telegramLink) && (
                <div style={{ display:'flex', gap:'0.5rem', marginBottom:'0.75rem' }}>
                  {groupLink    && <a href={groupLink}    target="_blank" rel="noreferrer" onClick={dismissNotif} style={{ flex:1, padding:'0.5rem', background:'rgba(37,211,102,0.1)', border:'1px solid rgba(37,211,102,0.3)', borderRadius:'0.5rem', color:'#25d366', textDecoration:'none', fontWeight:'700', fontSize:'0.78rem', textAlign:'center' }}>💬 WhatsApp</a>}
                  {telegramLink && <a href={telegramLink} target="_blank" rel="noreferrer" onClick={dismissNotif} style={{ flex:1, padding:'0.5rem', background:'rgba(0,136,204,0.1)', border:'1px solid rgba(0,136,204,0.3)', borderRadius:'0.5rem', color:'#0088cc', textDecoration:'none', fontWeight:'700', fontSize:'0.78rem', textAlign:'center' }}>✈️ Telegram</a>}
                </div>
              )}
              <button onClick={dismissNotif} style={{ width:'100%', padding:'0.75rem', background:'var(--bg-tertiary)', border:'1px solid var(--border-color)', borderRadius:'0.5rem', color:'var(--text-primary)', cursor:'pointer', fontWeight:'600' }}>
      ...
      <main className="main-content" style={{ paddingTop:'1rem' }}>
        ...
        <div style={{ background:'linear-gradient(135deg,#6366f1,#8b5cf6)', borderRadius:'1rem', padding:'1.25rem 1.5rem', marginBottom:'1.25rem', color:'white', boxShadow:'0 8px 25px rgba(99,102,241,0.3)' }}>
          <h2 style={{ fontSize:'1.35rem', fontWeight:'800', marginBottom:'0.25rem' }}>{greeting()}</h2>
          <p style={{ opacity:0.88, fontSize:'0.875rem' }}>Business summary — {rangeLabel}</p>
        </div>
        ...
        <div style={{ background:'rgba(245,158,11,0.1)', border:'1px solid rgba(245,158,11,0.35)', borderRadius:'0.875rem', padding:'0.875rem 1.125rem', marginBottom:'1.25rem', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'1rem', animation:'pulse 3s ease-in-out infinite' }}>
          ...
          <button onClick={() => navigate('/agent/terms')} style={{ whiteSpace:'nowrap', padding:'0.45rem 0.875rem', background:'rgba(245,158,11,0.2)', border:'1px solid rgba(245,158,11,0.4)', borderRadius:'0.5rem', color:'var(--warning)', cursor:'pointer', fontWeight:'700', fontSize:'0.78rem' }}>
            📋 Read Rules →
          </button>
        </div>

        {/* Platform Links - Clean organized section */}
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem', marginBottom:'1.25rem' }}>
          <div style={{ background:'linear-gradient(135deg,#10b981,#059669)', borderRadius:'1rem', padding:'1.25rem', color:'white' }}>
            <p style={{ opacity:0.85, fontSize:'0.75rem', marginBottom:'0.25rem' }}>Wallet Balance</p>
            <p style={{ fontSize:'1.65rem', fontWeight:'800', lineHeight:1, marginBottom:'0.2rem' }}>GH₵ {parseFloat(walletData.balance||0).toFixed(2)}</p>
            {walletData.pending_withdrawals > 0 && <p style={{ opacity:0.8, fontSize:'0.7rem' }}>Available: GH₵ {parseFloat(walletData.available||0).toFixed(2)}</p>}
            <button onClick={() => navigate('/agent/withdraw')} style={{ marginTop:'0.75rem', padding:'0.45rem 1rem', background:'rgba(255,255,255,0.2)', border:'1px solid rgba(255,255,255,0.35)', borderRadius:'0.5rem', color:'white', cursor:'pointer', fontWeight:'700', fontSize:'0.78rem', width:'100%' }}>💵 Withdraw</button>
          </div>
          <div style={{ background:'linear-gradient(135deg,#6366f1,#8b5cf6)', borderRadius:'1rem', padding:'1.25rem', color:'white' }}>
            <p style={{ opacity:0.85, fontSize:'0.75rem', marginBottom:'0.25rem' }}>{user?.store_name || 'My Store'}</p>
            ...
            <div style={{ display:'flex', gap:'0.35rem' }}>
              <button onClick={copyLink} style={{ flex:1, padding:'0.45rem 0.375rem', background:linkCopied?'rgba(255,255,255,0.35)':'rgba(255,255,255,0.2)', border:'1px solid rgba(255,255,255,0.35)', borderRadius:'0.5rem', color:'white', cursor:'pointer', fontWeight:'700', fontSize:'0.72rem' }}>
                {linkCopied ? '✓ Copied!' : '📋 Copy Link'}
              </button>
              {storeUrl && <button onClick={() => window.open(storeUrl,'_blank')} style={{ padding:'0.45rem 0.5rem', background:'rgba(255,255,255,0.15)', border:'1px solid rgba(255,255,255,0.3)', borderRadius:'0.5rem', color:'white', cursor:'pointer', fontSize:'0.85rem' }}>↗</button>}
            </div>
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'0.75rem', marginBottom:'1.25rem' }}>
          {[
            ...
            <div key={s.label} style={{ background:'var(--card-bg)', borderRadius:'0.875rem', padding:'1rem 0.75rem', border:'1px solid var(--border-color)', borderTop:`3px solid ${s.color}` }}>
              <p style={{ fontSize:'0.65rem', color:'var(--text-secondary)', marginBottom:'0.35rem', lineHeight:1.3 }}>{s.label}</p>
              <p style={{ fontSize:'1.05rem', fontWeight:'800', color:s.color }}>{s.unit}{s.value}</p>
            </div>
          ))}
        </div>
