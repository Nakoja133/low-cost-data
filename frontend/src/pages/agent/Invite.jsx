...
const AgentInvite = () => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)' }}>
      ...
      <main className="main-content" style={{ padding: '1.5rem', maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ 
          ...
          <h2 style={{ fontSize: '1.3rem', fontWeight: '800', marginBottom: '0.2rem', margin: 0 }}>Invite New Agent</h2>
          <p style={{ opacity: 0.88, fontSize: '0.85rem', margin: 0 }}>Create a sub-agent account and share their store link</p>
        </div>
          ...
          <form onSubmit={handleSubmit} autoComplete="off" style={{ display: 'flex', flexDirection: 'column', gap: '1.125rem' }}>
            ...
            {result && (
              <div style={{ 
                ...
                <p style={{ fontWeight: '700', marginBottom: result.storeLink ? '0.875rem' : 0, margin: 0 }}>
                  {result.message}
                </p>
                
                {result.storeLink && (
                  <>
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '0.4rem', marginTop: '0.5rem' }}>Store Link:</p>
                    <div style={{ 
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                      <button 
                        type="button" 
                        onClick={() => copy(result.storeLink, 'Store link copied!')} 
                        style={{ 
                          padding: '0.5rem 1rem', 
                      ...
                      {result.email && (
                        <button 
                          type="button" 
                          onClick={() => copy(`Email: ${result.email}\nStore: ${result.storeLink}`, 'Details copied!')} 
                          style={{ 
                            padding: '0.5rem 1rem', 
